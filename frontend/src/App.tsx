...
const API_BASE = 'http://localhost:8000/api'; // Or use docker internal host
...
function App() {
  ...
  const handlePlanetClick = async (nodeId: string) => {
    ...
    setNodes(prev => prev.map(n => 
      n.id === nodeId ? { ...n, status: newAction === 'kill' ? 'offline' : 'online' } : n
    ));
    ...
    try {
      ...
      // Revert if failed
      setNodes(prev => prev.map(n => 
        n.id === nodeId ? { ...n, status: node.status } : n
      ));
    }
  };
    ...
    <div className="app-container">
      ...
      <div className="sidebar glass-panel">
        <div className="header">
          ...
          <div>
            <h1>Relic Ring</h1>
            <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>ZETA-26 PROTOCOL DASHBOARD</div>
          </div>
        </div>

        <div className="control-panel">
          <h2 className="text-gradient">Routing Dashboard</h2>
          <form onSubmit={handleRoute} className="form-group" style={{ gap: '16px', marginTop: '16px' }}>
            <div className="form-group">
              <label>Origin Node</label>
              <select className="form-control" value={origin} onChange={(e) => setOrigin(e.target.value)}>
                {nodes.map(n => <option key={`orig-${n.id}`} value={n.id}>{n.id}</option>)}
              </select>
            </div>
            ...
            <div className="form-group">
              ...
              <select className="form-control" value={destination} onChange={(e) => setDestination(e.target.value)}>
                {nodes.map(n => <option key={`dest-${n.id}`} value={n.id}>{n.id}</option>)}
              </select>
            </div>
            ...
            {error && <div style={{ color: 'var(--secondary-color)', fontSize: '0.9rem' }}>{error}</div>}
          </form>
        </div>

        <div className="log-panel">
          <h3><Activity size={20} /> Transmission Logs {totalLatency && `(${totalLatency.toFixed(4)}s)`}</h3>
          <div className="log-content">
            ...
            {hopLog.map((hop, idx) => (
              <div key={idx} className="hop-item">
                <div className="hop-header">Hop {hop.hop_number}: {hop.from_planet} → {hop.to_planet}</div>
                ...
                <div className="hop-detail">
                  <span>Translation ({hop.data_translation.next_hop_codex}):</span>
                  <span style={{ color: 'var(--primary-color)' }}>{hop.data_translation.binary_transmission_stream}</span>
                </div>
                <div className="hop-detail" style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '4px', marginTop: '4px' }}>
                  <span>Latency:</span>
                  <span>{hop.latency_breakdown.total_hop_latency_sec.toFixed(4)}s</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="main-view glass-panel">
        <div className="canvas-container">
          <div className="overlay-status">
            ...
            <button className="btn-primary" style={{ padding: '4px 8px', fontSize: '0.7rem', marginLeft: '12px' }} onClick={fetchUniverse}>
          ...
          <div style={{ position: 'absolute', bottom: 20, left: 20, color: 'var(--text-secondary)', fontSize: '0.8rem', background: 'rgba(0,0,0,0.5)', padding: '8px', borderRadius: '4px' }}>
            <Zap size={14} style={{ verticalAlign: 'middle', marginRight: '4px', color: 'var(--secondary-color)' }} />
            Chaos Mode: Click on any planet to simulate a catastrophic hardware failure.
          </div>
        </div>
      </div>
    </div>
